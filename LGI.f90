...
!functions (find the end of inflation)
  type transfert
     ...
     real(SP) :: real1, real2, real3, real4, real5, real6, real7, real8, real9, real10
     complex(SP) :: complex1, complex2, complex3, complex4, complex5, &
                    complex6, complex7, complex8, complex9, complex10
     real(SP), dimension(:), pointer :: ptrvector1 => null()
     real(SP), dimension(:), pointer :: ptrvector2 => null()
  end type transfert

	type(transfert)  :: params

    print*, K3best(0.68,0.1,0.7,0.1,0.74,0.1)

	CONTAINS


! Returns the two-time correlator of Larsson spin variables on the two mode squeezed state with 
! squeezing parameters ra and phia at time ta and rb and phib at time tb, given ell.
   FUNCTION Cab(ra,phia,rb,phib,l)
        REAL(SP) :: Cab
        REAL(SP), INTENT(IN) :: ra,phia,rb,phib,l
        COMPLEX(SP) :: calAloc, Aloc, Bloc, Cloc, Cabloc
        INTEGER ::n,m,Npointsn, Npointsm
        calAloc = calA(ra,phia,rb,phib)
        Aloc = A(ra,phia,rb,phib)
        Bloc = B(ra,phia,rb,phib)
        Cloc = C(ra,phia,rb,phib)
        Npointsn = Nn(Aloc,Bloc,Cloc,l)
        Npointsm = Nm(Aloc,Bloc,Cloc,l)
        Cabloc = 0._sp
        if (ra==rb .and. phia==phib) then
            Cab=1.
        else
            do n = -Npointsn, Npointsn
                do m = -Npointsm, Npointsm
                    Cabloc=Cabloc+(-1)**(n+m)* &
                    !integralQuad(ra,phia,rb,phib,real(n),real(m),l,Aloc,Bloc,Cloc)
                    integralGauss(ra,phia,rb,phib,real(n),real(m),l,Aloc,Bloc,Cloc)
                end do
            end do
            Cab = realpart(cmplx(0._sp,1._sp)/2._sp*sqrt(pi/Bloc)*calAloc*Cabloc)
        end if
    END FUNCTION Cab

! Returns the max and min of K3 and K3', over ell, given ra,phia,rb,phib,rc,phic
    FUNCTION K3best(ra,phia,rb,phib,rc,phic)
        REAL(SP), INTENT(IN) :: ra,phia,rb,phib,rc,phic
        REAL(SP):: l, lmin, lmax, CCab, CCac, CCbc, K3, K3prime, deltalog2l
        REAL(SP) :: K3min, K3max, K3primemin, K3primemax, lK3min, lK3max, lK3primemin, lK3primemax
        REAL(SP) :: K3best(8)
        INTEGER :: i, Npoints, Npointsrefined
        Npoints = 80
        lmin = minll(ra,phia,rb,phib,rc,phic)
        lmax = maxll(ra,phia,rb,phib,rc,phic)
        deltalog2l = log(lmax/lmin)/log(2.)/real(Npoints,sp)
        Npointsrefined = 10
        K3min = 10.
        K3max = -10.
        K3primemin = 10.
        K3primemax = -10.
        DO i=1,Npoints
            l = lmin*(lmax/lmin)**(real(i)/real(Npoints))
            CCab = Cab(ra,phia,rb,phib,l)
            CCbc = Cab(rb,phib,rc,phic,l)
            CCac = Cab(ra,phia,rc,phic,l)
            K3 = CCab+CCbc-CCac
            K3prime = -CCab-CCbc-CCac
            if (K3 .ge. K3max) then
                K3max = K3
                lK3max = l
            end if
            ...
            if (K3prime .ge. K3primemax) then
                K3primemax = K3prime
                lK3primemax = l
            end if
            if (K3prime .le. K3primemin) then
                K3primemin = K3prime
                lK3primemin = l
            end if
        END DO
        ! refine for K3min
        lmin = lK3min*2**(-deltalog2l)
        lmin = lK3min*2**(deltalog2l)
        DO i=1,Npointsrefined
            l = lmin*(lmax/lmin)**(real(i)/real(Npointsrefined))
            CCab = Cab(ra,phia,rb,phib,l)
            CCbc = Cab(rb,phib,rc,phic,l)
            CCac = Cab(ra,phia,rc,phic,l)
            K3 = CCab+CCbc-CCac
        ...
        END DO
        ! refine for K3primemin
        lmin = lK3primemin*2**(-deltalog2l)
        lmin = lK3primemin*2**(deltalog2l)
        DO i=1,Npointsrefined
            l = lmin*(lmax/lmin)**(real(i)/real(Npointsrefined))
            CCab = Cab(ra,phia,rb,phib,l)
        ...
        END DO
        ! refine for K3primemax
        lmin = lK3primemax*2**(-0.1)
        lmin = lK3primemax*2**(0.1)
        DO i=1,Npointsrefined
            l = lmin*(lmax/lmin)**(real(i)/real(Npointsrefined))
            CCab = Cab(ra,phia,rb,phib,l)
        ...
        K3best(1) = lK3min
        K3best(2) = lK3max
        K3best(3) = lK3primemin
        K3best(4) = lK3primemax
        K3best(5) = K3min
        K3best(6) = K3max
        K3best(7) = K3primemin
        K3best(8) = K3primemax
    ...
    FUNCTION detM(ra,phia,rb,phib)
        COMPLEX(SP) :: detM
        REAL(SP), INTENT(IN) :: ra,phia,rb,phib
        detM = -128._sp*cmplx(0._sp,1._sp)*(sin(2._sp*phia)*tanh(ra)-sin(2._sp*phib)*tanh(rb) &
                -sin(2._sp*(phia-phib))*tanh(ra)*tanh(rb))
    END FUNCTION detM

    FUNCTION A(ra,phia,rb,phib)
        COMPLEX(SP) :: A
        REAL(SP), INTENT(IN) :: ra,phia,rb,phib
        A = -64._sp/detM(ra,phia,rb,phib)*(1._sp-exp(-2._sp*phia*cmplx(0._sp,1._sp))*tanh(ra)+ &
            exp(-2._sp*phib*cmplx(0._sp,1._sp))*tanh(rb)- &
            exp(2._sp*cmplx(0._sp,1._sp)*(phia-phib))*tanh(ra)*tanh(rb))- &
            1._sp/(1._sp-exp(-2._sp*phia*cmplx(0._sp,1._sp))*tanh(ra))
    END FUNCTION A

    FUNCTION B(ra,phia,rb,phib)
        COMPLEX(SP) :: B
        REAL(SP), INTENT(IN) :: ra,phia,rb,phib
        B = -64._sp/detM(ra,phia,rb,phib)*(1._sp+exp(2._sp*phia*cmplx(0._sp,1._sp))*tanh(ra)- &
            exp(2._sp*phib*cmplx(0._sp,1._sp))*tanh(rb)- &
            exp(2._sp*cmplx(0._sp,1._sp)*(phia-phib))*tanh(ra)*tanh(rb))- &
            1._sp/(1._sp-exp(2._sp*phib*cmplx(0._sp,1._sp))*tanh(rb))
    END FUNCTION B

    FUNCTION C(ra,phia,rb,phib)
        COMPLEX(SP) :: C
        REAL(SP), INTENT(IN) :: ra,phia,rb,phib
        C = 128._sp/(detM(ra,phia,rb,phib)*cosh(ra)*cosh(rb))
    END FUNCTION C

    FUNCTION calA(ra,phia,rb,phib)
        COMPLEX(SP) :: calA
        REAL(SP), INTENT(IN) :: ra,phia,rb,phib
        calA = 8._sp/(sqrt(detM(ra,phia,rb,phib))*pi*cosh(ra)*cosh(rb)* &
                sqrt((1._sp-exp(-2._sp*phia*cmplx(0._sp,1._sp))*tanh(ra))* &
                (1._sp-exp(2._sp*phib*cmplx(0._sp,1._sp))*tanh(rb))))
    END FUNCTION calA

    FUNCTION integrand(ra,phia,rb,phib,Q,n,m,l,Aloc,Bloc,Cloc)
        COMPLEX(SP) :: integrand
        REAL(SP), INTENT(IN) :: ra,phia,rb,phib,Q,n,m,l
        COMPLEX(SP), INTENT(IN) :: Aloc,Bloc,Cloc
        integrand = moderf1(Aloc,Bloc,Cloc,Q,l,real(m))-moderf1(Aloc,Bloc,Cloc,Q,l,real(m+1))
    END FUNCTION integrand

    FUNCTION integrandTransferReal(Q)
        REAL(SP) :: integrandTransferReal
        REAL(SP), INTENT(IN) :: Q
        REAL(SP):: ra,phia,rb,phib,n,m,l
        COMPLEX(SP) :: Aloc,Bloc,Cloc
        ra = params%real1
        phia = params%real2
        rb = params%real3
        phib = params%real4
        ...
        m = params%real6
        l = params%real7
        Aloc = params%complex1
        Bloc = params%complex2
        Cloc = params%complex3
        integrandTransferReal = realpart(integrand(ra,phia,rb,phib,Q,n,m,l,Aloc,Bloc,Cloc))
    END FUNCTION integrandTransferReal

    FUNCTION integrandTransferImag(Q)
        REAL(SP) :: integrandTransferImag
        REAL(SP), INTENT(IN) :: Q
        REAL(SP):: ra,phia,rb,phib,n,m,l
        COMPLEX(SP) :: Aloc,Bloc,Cloc
        ra = params%real1
        ...
        Bloc = params%complex2
        Cloc = params%complex3
        integrandTransferImag = imagpart(integrand(ra,phia,rb,phib,Q,n,m,l,Aloc,Bloc,Cloc))
    END FUNCTION integrandTransferImag

! Integrates with a mere Riemann sum. Long but under good numerical control
    FUNCTION integral(ra,phia,rb,phib,n,m,l,Aloc,Bloc,Cloc)
        COMPLEX(SP) :: integral
        REAL(SP), INTENT(IN) :: ra,phia,rb,phib,n,m,l
        COMPLEX(SP), INTENT(IN) :: Aloc,Bloc,Cloc
        REAL(SP) :: Qloc
        INTEGER :: i, Nstep
        Nstep = max(100,INT(min(1000*max(1._sp,l/abs(sqrt(Bloc)/Cloc)),1000000.)))
        integral = 0._sp
        do i = 1, Nstep
            Qloc = (n+real(i)/real(Nstep))*l
            integral=integral+integrand(ra,phia,rb,phib,Qloc,n,m,l,Aloc,Bloc,Cloc)
        end do
        integral = integral*l/real(Nstep)
    END FUNCTION integral

! Integrates with Quadrature methods (fast)
    FUNCTION integralQuad(ra,phia,rb,phib,n,m,l,Aloc,Bloc,Cloc)
        COMPLEX(SP) :: integralQuad
        REAL(SP), INTENT(IN) :: ra,phia,rb,phib,n,m,l
        COMPLEX(SP), INTENT(IN) :: Aloc,Bloc,Cloc
        REAL(SP) :: epsabs, epsrel, resultreal, resultimag, abserr
        INTEGER :: i, Nstep, neval, ier
        Nstep = max(100,INT(min(1000*max(1._sp,l/abs(sqrt(Bloc)/Cloc)),1000000.)))
        epsabs = 10._sp**(-4.)!epsilon(1._sp)
        epsrel = 10._sp**(-2.)!epsilon(1._sp)
        params%real1 = ra
        params%real2 = phia
        params%real3 = rb
        params%real4 = phib
        ...
        params%real6 = m
        params%real7 = l
        params%complex1 = Aloc
        params%complex2 = Bloc
        params%complex3 = Cloc
        call qags (integrandTransferReal,n*l,(n+1.)*l,epsabs,epsrel,resultreal,abserr,neval,ier)
        call qags (integrandTransferImag,n*l,(n+1.)*l,epsabs,epsrel,resultimag,abserr,neval,ier)
        integralQuad = resultreal + cmplx(0.,1.)*resultimag
    END FUNCTION integralQuad

! Integrates with Gaussian Quadrature (even faster)
    FUNCTION integralGauss(ra,phia,rb,phib,n,m,l,Aloc,Bloc,Cloc)
        COMPLEX(SP) :: integralGauss
        REAL(SP), INTENT(IN) :: ra,phia,rb,phib,n,m,l
        COMPLEX(SP), INTENT(IN) :: Aloc,Bloc,Cloc
        REAL(SP) :: resultreal, resultimag
        INTEGER :: i, Neval
        Neval = int(40+10.*l)
        params%real1 = ra
        params%real2 = phia
        ...
        params%complex2 = Bloc
        params%complex3 = Cloc
        call GaussInt(integrandTransferReal,n*l,(n+1.)*l,Neval,resultreal)
        call GaussInt(integrandTransferImag,n*l,(n+1.)*l,Neval,resultimag)
        integralGauss = resultreal + cmplx(0.,1.)*resultimag
    END FUNCTION integralGauss


    FUNCTION Nn(Aloc,Bloc,Cloc,l)
        COMPLEX(SP), INTENT(IN) :: Aloc,Bloc,Cloc
        REAL(SP), INTENT(IN) :: l
        INTEGER :: Nn
        Nn = Min(20,int(sqrt(abs(20./realpart(Aloc)))/l)+2)
    END FUNCTION Nn

    FUNCTION Nm(Aloc,Bloc,Cloc,l)
        COMPLEX(SP), INTENT(IN) :: Aloc,Bloc,Cloc
        REAL(SP), INTENT(IN) :: l
        INTEGER :: Nm
        Nm = Min(20,int(sqrt(abs(20./realpart(Bloc)))/l)+2)
    END FUNCTION Nm

    FUNCTION maxl(Aloc,Bloc,Cloc)
        COMPLEX(SP), INTENT(IN) :: Aloc,Bloc,Cloc
        REAL(SP) :: maxl
        maxl = (max(sqrt(abs(20./realpart(Aloc))),sqrt(abs(20./realpart(Bloc)))))
    END FUNCTION maxl

    FUNCTION minl(Aloc,Bloc,Cloc)
        COMPLEX(SP), INTENT(IN) :: Aloc,Bloc,Cloc
        REAL(SP) :: minl
        minl = (min(sqrt(abs(0.05/realpart(Aloc))),sqrt(abs(0.05/realpart(Bloc))), &
                sqrt(abs(0.05/imagpart(Aloc))),sqrt(abs(0.05/imagpart(Bloc)))))
    END FUNCTION minl

    FUNCTION minll(ra,phia,rb,phib,rc,phic)
        REAL(SP), INTENT(IN) :: ra,phia,rb,phib,rc,phic
        REAL(SP) :: minll, lminab, lminbc, lminac
        COMPLEX(SP) :: Aloc, Bloc, Cloc
        Aloc = A(ra,phia,rb,phib)
        Bloc = B(ra,phia,rb,phib)
        Cloc = C(ra,phia,rb,phib)
        lminab=minl(Aloc,Bloc,Cloc)
        Aloc = A(ra,phia,rc,phic)
        Bloc = B(ra,phia,rc,phic)
        Cloc = C(ra,phia,rc,phic)
        lminac=minl(Aloc,Bloc,Cloc)
        Aloc = A(rc,phic,rb,phib)
        Bloc = B(rc,phic,rb,phib)
        Cloc = C(rc,phic,rb,phib)
        lminbc=minl(Aloc,Bloc,Cloc)
        minll = min(lminab, lminbc, lminac)
    END FUNCTION minll

    FUNCTION maxll(ra,phia,rb,phib,rc,phic)
        REAL(SP), INTENT(IN) :: ra,phia,rb,phib,rc,phic
        REAL(SP) :: maxll, lmaxab, lmaxbc, lmaxac
        COMPLEX(SP) :: Aloc, Bloc, Cloc
        Aloc = A(ra,phia,rb,phib)
        Bloc = B(ra,phia,rb,phib)
        Cloc = C(ra,phia,rb,phib)
        lmaxab=maxl(Aloc,Bloc,Cloc)
        Aloc = A(ra,phia,rc,phic)
        Bloc = B(ra,phia,rc,phic)
        Cloc = C(ra,phia,rc,phic)
        lmaxac=maxl(Aloc,Bloc,Cloc)
        Aloc = A(rc,phic,rb,phib)
        Bloc = B(rc,phic,rb,phib)
        Cloc = C(rc,phic,rb,phib)
        lmaxbc=maxl(Aloc,Bloc,Cloc)
        maxll = max(lmaxab, lmaxbc, lmaxac)
    END FUNCTION maxll

! error function
! returns erf(z)exp(argexp), cf (7.1.29) of Abramowit and Stegun, where z = x+iy
    FUNCTION moderf1(Aloc,Bloc,Cloc,Q,l,m)
        COMPLEX(SP) :: moderf1, sum, argexp, fn, gn, z, costerm, erfloc
        COMPLEX(SP), INTENT(IN) :: Aloc,Bloc,Cloc
        REAL(SP), INTENT(IN) ::  Q,l,m
        REAL(SP) :: x,y,nn
        INTEGER :: n, Nasym, nmax, nmin, n0
        z = cmplx(0._sp,1._sp)*(sqrt(Bloc)*l*m+0.5_sp*Cloc/sqrt(Bloc)*Q)
        x = realpart(z)
        y = imagpart(z)
        argexp = -Cloc**2*Q**2/(4._sp*Bloc)+Aloc*Q**2
        Nasym=10 ! number of terms in the sum
        n0 = int(2.*abs(y)) ! value of n where the generic term od the sum is maximal
        nmin=max(1,n0-Nasym/2)
        nmax=max(Nasym,n0+Nasym/2)
        sum=0.
        do n=nmin,nmax
            nn=real(n)
            fn=2.*x*exp(argexp-nn**2/4.-x**2)-x*(exp(argexp+nn*y-nn**2/4.-x**2)+ &
                    exp(argexp-nn*y-nn**2/4.-x**2))*cos(2.*x*y)+nn/2.*(exp(argexp+nn*y-nn**2/4.-x**2)- &
                    exp(argexp-nn*y-nn**2/4.-x**2))*sin(2.*x*y)
            gn=x*(exp(argexp+nn*y-nn**2/4.-x**2)+exp(argexp-nn*y-nn**2/4.-x**2))*sin(2.*x*y)+ &
                    nn/2.*(exp(argexp+nn*y-nn**2/4.-x**2)-exp(argexp-nn*y-nn**2/4.-x**2))*cos(2.*x*y)
            sum=sum+(fn+cmplx(0.,1.)*gn)/(nn**2+4.*x**2)
        end do
        if (x .eq. 0.) then
            costerm = cmplx(0.,1.)*y/pi*exp(argexp)
        else
            costerm = exp(-x**2+argexp)*(1.-cos(2.*x*y)+cmplx(0._sp,1._sp)* &
                    sin(2.*x*y))/(2.*pi*x)
        endif
        moderf1 = 2./pi*sum+costerm+erf(x)*exp(argexp)
    END FUNCTION moderf1
...
!  Parameters:
!
!    Input, complex (SP) Z, the argument.
!
!    Output, complex (SP) CER, the function value.
!
  implicit none
...
!    Output, integer ( kind = 4 ) NEVAL, the number of times the integral was evaluated.
!
!    Output, integer ( kind = 4 ) IER, return flag.
...
!
  ...
  integer ( kind = 4 ) ktmin
  integer ( kind = 4 ) last
  integer ( kind = 4 ) levcur
  integer ( kind = 4 ) level(limit)
  integer ( kind = 4 ) levmax
  integer ( kind = 4 ) maxerr
  integer ( kind = 4 ) ndin(40)
  integer ( kind = 4 ) neval
  integer ( kind = 4 ) nint
  logical noext
  integer ( kind = 4 ) npts
...
!    Input, real ( kind = 4 ) A, B, the limits of integration.
!
!    Input, real ( kind = 4 ) C, a parameter in the weight function, which must
...
!    Input, real ( kind = 4 ) A, B, the limits of integration.
!
!    Input, real ( kind = 4 ) C, a parameter in the weight function, which cannot be
!    equal to A or B.
!
!    Input, real ( kind = 4 ) EPSABS, EPSREL, the absolute and relative accuracy requested.
!
!    Input, integer ( kind = 4 ) LIMIT, the upper bound on the number of subintervals that
...
!    which evaluates the integrand function.
!
!    Input, real ( kind = 4 ) A, the lower limit of integration.
!
!    Input, real ( kind = 4 ) OMEGA, the parameter in the weight function.
!
!    Input, integer ( kind = 4 ) INTEGR, indicates which weight functions is used
!    = 1, w(x) = cos(omega*x)
!    = 2, w(x) = sin(omega*x)
!
...
!
  ...
  real ( kind = 4 ) alist(limit)
  real ( kind = 4 ) blist(limit)
  real ( kind = 4 ) chebmo(maxp1,25)
  real ( kind = 4 ) elist(limit)
  real ( kind = 4 ) epsabs
  real ( kind = 4 ) erlst(limlst)
  real ( kind = 4 ), external :: f
  integer ( kind = 4 ) ier
  integer ( kind = 4 ) integr
  integer ( kind = 4 ) iord(limit)
  integer ( kind = 4 ) ierlst(limlst)
  integer ( kind = 4 ) lst
  integer ( kind = 4 ) neval
  integer ( kind = 4 ) nnlog(limit)
  real ( kind = 4 ) omega
  real ( kind = 4 ) result
  real ( kind = 4 ) rlist(limit)
  real ( kind = 4 ) rslst(limlst)
...
!    Input, real ( kind = 4 ) OMEGA, the parameter in the weight function.
!
!    Input, integer ( kind = 4 ) INTEGR, indicates which weight function is used
!    = 1      w(x) = cos(omega*x)
!    = 2      w(x) = sin(omega*x)
!
!    Input, real ( kind = 4 ) EPSABS, the absolute accuracy requested.
...
! real ( kind = 4 ) dla
  ...
  integer ( kind = 4 ) numrl2
  real ( kind = 4 ) omega
  real ( kind = 4 ), parameter :: p = 0.9E+00
  real ( kind = 4 ), parameter :: pi = 3.1415926535897932E+00
  real ( kind = 4 ) p1
  real ( kind = 4 ) psum(52)
  real ( kind = 4 ) reseps
  real ( kind = 4 ) result
...
!    Input, real ( kind = 4 ) OMEGA, the parameter in the weight function.
!
!    Input, integer ( kind = 4 ) INTEGR, specifies the weight function:
!    1, W(X) = cos ( OMEGA * X )
!    2, W(X) = sin ( OMEGA * X )
!
!    Input, real ( kind = 4 ) EPSABS, EPSREL, the absolute and relative accuracy requested.
...
!    Input, real ( kind = 4 ) A, B, the limits of integration.
!
!    Input, real ( kind = 4 ) ALFA, BETA, parameters used in the weight function.
!    ALFA and BETA should be greater than -1.
!
!    Input, integer ( kind = 4 ) INTEGR, indicates which weight function is to be used
!    = 1  (x-a)**alfa*(b-x)**beta
...
!    Input, real ( kind = 4 ) A, B, the limits of integration.
!
!    Input, real ( kind = 4 ) C, the parameter in the weight function.
!
!    Output, real ( kind = 4 ) RESULT, the estimated value of the integral.
...
!
  ...
  real ( kind = 4 ) cc
  real ( kind = 4 ) centr
  real ( kind = 4 ) cheb12(13)
  real ( kind = 4 ) cheb24(25)
  real ( kind = 4 ), external :: f
  real ( kind = 4 ) fval(25)
  real ( kind = 4 ) hlgth
  integer ( kind = 4 ) i
  ...
  real ( kind = 4 ) res24
  real ( kind = 4 ) u
  real ( kind = 4 ), parameter, dimension ( 11 ) :: x = (/ &
...
!    = 2, w(x) = sin(omega*x)
!
!    ?, integer ( kind = 4 ) NRMOM, the length of interval (a,b) is equal to the length
...
!    Output, integer ( kind = 4 ) NEVAL, the number of times the integral was evaluated.
!
!    Output, real ( kind = 4 ) RESABS, approximation to the integral J.
!
!    Output, real ( kind = 4 ) RESASC, approximation to the integral of abs(F-I/(B-A)).
!
!         on entry and return
...
!
  ...
  integer ( kind = 4 ) momcom
  integer ( kind = 4 ) neval
  integer ( kind = 4 ), parameter :: nmac = 28
  integer ( kind = 4 ) noeq1
  integer ( kind = 4 ) noequ
...
!    Input, real ( kind = 4 ) A, B, the limits of integration.
!
!    Input, real ( kind = 4 ) BL, BR, the lower and upper limits of integration.
!    A <= BL < BR <= B.
!
...
!    Output, real ( kind = 4 ) ABSERR, an estimate of || I - RESULT ||.
!
!    Output, real ( kind = 4 ) RESASC, approximation to the integral of abs(F*W-I/(B-A)).
!
!    Input, integer ( kind = 4 ) INTEGR,  determines the weight function
...
!
!    Output, real ( kind = 4 ) CHEB12(13), the Chebyshev coefficients for degree 12.
!
!    Output, real ( kind = 4 ) CHEB24(25), the Chebyshev coefficients for degree 24.
!
  implicit none
...
!    Input, real ( kind = 4 ) A, B, the limits of integration.
!
!    Input, real ( kind = 4 ) OMEGA, the multiplier of X in the weight function.
!
!    Input, integer ( kind = 4 ) INTEGR, indicates the weight functions to be used.
!    = 1, w(x) = cos(omega*x)
!    = 2, w(x) = sin(omega*x)
!
!    Input, real ( kind = 4 ) EPSABS, EPSREL, the absolute and relative accuracy requested.
!
!    Input, integer ( kind = 4 ) LIMIT, the maximum number of subintervals of [A,B]
...
!                       calling program.
!
!    ?, real ( kind = 4 ) P1, P2, P3, P4, parameters in the weight function
!
!    Input, integer ( kind = 4 ) KP, key for indicating the type of weight function
!
!    Input, real ( kind = 4 ) A, B, the limits of integration.
!
!    Output, real ( kind = 4 ) RESULT, the estimated value of the integral.
...
!
  ...
  real ( kind = 4 ) reskh
  real ( kind = 4 ) result
  real ( kind = 4 ), external :: w
  real ( kind = 4 ), dimension ( 4 ) :: wg = (/ &
    1.294849661688697E-01,     2.797053914892767E-01, &
...
!  Parameters:
!
!    Input, real ( kind = 4 ) ALFA, a parameter in the weight function w(x), ALFA > -1.
!
!    Input, real ( kind = 4 ) BETA, a parameter in the weight function w(x), BETA > -1.
!
!           ri     - real
...
!  Parameters:
!
!    Input, real ( kind = 4 ) X, the point at which the weight function is evaluated.
!
!    Input, real ( kind = 4 ) C, the location of the singularity.
!
!    Input, real ( kind = 4 ) P2, P3, P4, parameters that are not used.
!
!    Input, integer ( kind = 4 ) KP, a parameter that is not used.
!
!    Output, real ( kind = 4 ) QWGTC, the value of the weight function at X.
!
  implicit none
...
!    Input, real ( kind = 4 ) X, the point at which the weight function is evaluated.
!
!    Input, real ( kind = 4 ) OMEGA, the factor multiplying X.
!
!    Input, real ( kind = 4 ) P2, P3, P4, parameters that are not used.
!
!    Input, integer ( kind = 4 ) INTEGR, specifies which weight function is used:
!    1. W(X) = cos ( OMEGA * X )
!    2, W(X) = sin ( OMEGA * X )
!
!    Output, real ( kind = 4 ) QWGTO, the value of the weight function at X.
!
  implicit none
...
!    Input, real ( kind = 4 ) X, the point at which the weight function is evaluated.
!
!    Input, real ( kind = 4 ) A, B, the endpoints of the integration interval.
!
!    Input, real ( kind = 4 ) ALFA, BETA, exponents that occur in the weight function.
!
!    Input, integer ( kind = 4 ) INTEGR, specifies which weight function is used:
!    1. W(X) = (X-A)**ALFA * (B-X)**BETA
!    2, W(X) = (X-A)**ALFA * (B-X)**BETA * log (X-A)
...
!    4, W(X) = (X-A)**ALFA * (B-X)**BETA * log (X-A) * log(B-X)
!
!    Output, real ( kind = 4 ) QWGTS, the value of the weight function at X.
!
  implicit none
...
!
 ...
 12	continue
    ...
    subroutine GaussInt (f, a, b, Npoints, result)
        real(sp), intent(in) :: a, b
        real(sp), intent(inout) :: result
        integer, intent(in) :: Npoints
        real (sp), external :: f
        real(sp) :: x(Npoints), w(Npoints)
        integer :: i
        call gauleg(a,b,x,w,Npoints)
        result = 0._sp
        do i = 1, Npoints
